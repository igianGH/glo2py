...
def interpreter(file="source",developer=False,dline=True,smart=True,report=False,randIN=True,test=False):
  ...
  try:
    ...
    elif("no attribute \'typos\'" in trb):
      errmsg2+="ΣΥΝΤΑΚΤΙΚΟ ΣΦΑΛΜΑ:"
      errmsg2+="\n> ΜΗ ΕΓΚΥΡΗ ΣΥΝΤΑΞΗ, αυτό το αντικείμενο δεν είναι ΜΕΤΑΒΛΗΤΗ"
...
def main():
  ...
  try:
    ...
    pass\n''')
    flist=",".join([f for f in vdict.keys() if f!="_main_"])+",".join(vlist)   #segVar
    if(flist!=""):
      fout.write('''
def delete():
  global '''+xpr(flist))
